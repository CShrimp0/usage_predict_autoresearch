...
from utils.paths import build_run_suffix, create_run_dir, write_command
...
def initialize_run(
    ...
    include_model_in_name: bool | None = None,
):
    outputs_root = config.get("paths", {}).get("outputs_root", "outputs")
    experiment = experiment_name or config.get("experiment", {}).get("name", "experiment")
    model_name = config.get("model", {}).get("name")
    include_model = (
        config.get("paths", {}).get("include_model_in_run_name", True)
        if include_model_in_name is None
        else include_model_in_name
    )
    suffix = build_run_suffix([experiment, model_name if include_model else None])
    run_dir = create_run_dir(outputs_root, suffix)
