...
class AgeRegressor(nn.Module):
    def __init__(self, cfg: ExperimentConfig, aux_input_dim: int) -> None:
        ...
        if aux_input_dim > 0:
            ...
            self.film_gamma_scale = nn.Parameter(torch.tensor(0.1))
            self.film_beta_scale = nn.Parameter(torch.tensor(0.1))
    ...
    def forward(self, images: torch.Tensor, aux_features: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.aux_branch is not None and aux_features is not None:
            ...
            gamma_scale = self.film_gamma_scale.clamp(0.0, 0.5)
            beta_scale = self.film_beta_scale.clamp(0.0, 0.5)
            image_features = image_features * (1.0 + gamma_scale * torch.tanh(gamma)) + beta_scale * beta
